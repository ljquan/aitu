...
/**
 * 从图片中提取主色调（出现最多的颜色）
 * 使用简化的颜色量化算法，将颜色空间量化后统计频率
 *
 * @param imageBitmap 图片位图
 * @param sampleSize 采样尺寸（缩小图片以提高性能）
...
async function extractDominantColor(
  ...
  const sampleWidth = aspectRatio >= 1 ? sampleSize : Math.round(sampleSize * aspectRatio);
  const sampleHeight = aspectRatio >= 1 ? Math.round(sampleSize / aspectRatio) : sampleSize;
  ...
  // 颜色量化：将颜色空间划分为更大的区间（每个通道 16 个级别）
...
/**
 * 将图片适配到目标画布尺寸（保持宽高比，居中放置，背景填充主色调）
 *
 * 处理规则：
 * 1. 创建精确匹配目标尺寸的画布
 * 2. 提取参考图主色调作为背景色
 * 3. 缩放图片到画布可容纳的最大尺寸（保持宽高比）
 * 4. 将图片居中放置在画布上
 *
 * @param blob 原始图片 Blob
...
export async function fitImageToCanvas(
  ...
  try {
    ...
    // 提取主色调作为背景色
    const dominantColor = await extractDominantColor(imageBitmap);

    // 计算缩放比例（使用 contain 模式，确保图片完全显示）
    ...
    const ctx = canvas.getContext('2d');
    if (!ctx) {
      console.warn('[MediaUtils] Failed to get 2d context for fitImageToCanvas');
      imageBitmap.close();
      return blob;
    }
    ...
    ctx.drawImage(
      imageBitmap,
      0, 0, srcWidth, srcHeight, // 源区域（整张图片）
      offsetX, offsetY, scaledWidth, scaledHeight // 目标区域（居中放置）
    );
    imageBitmap.close();

    // 转换为 Blob（使用 PNG 以保持质量）
    const resultBlob = await canvas.convertToBlob({ type: 'image/png' });

    console.log(
      `[MediaUtils] Fit image from ${srcWidth}x${srcHeight} to ${targetWidth}x${targetHeight} ` +
      `(scaled: ${scaledWidth}x${scaledHeight}, offset: ${offsetX},${offsetY}, ` +
      `bg: rgb(${dominantColor.r},${dominantColor.g},${dominantColor.b}))`
    );

    return resultBlob;
  } catch (err) {
    console.warn('[MediaUtils] fitImageToCanvas failed, returning original:', err);
    return blob;
  }
}
