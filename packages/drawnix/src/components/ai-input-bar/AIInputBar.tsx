...
export const AIInputBar: React.FC<AIInputBarProps> = React.memo(
  ({ className, isDataReady }) => {
    ...
    const fileToBase64WithDimensions = useCallback(
      (file: File): Promise<{ url: string; width: number; height: number }> => {
    ...
    const handleWorkflowRetry = useCallback(
      async (
        ...
        // 更新 ChatDrawer 显示
        updateWorkflowMessageRef.current(
          toWorkflowMessageData(workflowDefinition, retryContext)
        );
        ...
        const executeStep = async (
          ...
          workflowControl.updateStep(step.id, 'running');
          updateWorkflowMessageRef.current(
            toWorkflowMessageData(workflowControl.getWorkflow()!, retryContext)
          );
