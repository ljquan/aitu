...
export class FallbackMediaExecutor implements IMediaExecutor {
  ...
  async generateImage(
    ...
    try {
      ...
      const result = this.parseImageResponse(data);
      const duration = Date.now() - startTime;
  ...
  async generateVideo(
    ...
    try {
      ...
      // 轮询等待视频完成
      const result = await this.pollVideoStatus(
        videoId,
        config.videoConfig,
        (progress) => {
  ...
  async aiAnalyze(
    ...
    // 提取 prompt 用于日志记录
    const logPrompt = this.extractPromptFromMessages(chatMessages);
   ...
   */
  private extractPromptFromMessages(messages: Array<{ role: string; content: unknown }>): string {
   ...
   * 构建图片生成请求体
   */
  private buildImageRequestBody(params: {
    prompt: string;
    model: string;
    size?: string;
   ...
   * 解析图片生成响应
   */
  private parseImageResponse(data: any): { url: string; urls?: string[] } {
    // 支持多种响应格式
