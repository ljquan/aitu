...
import { mergeVideos, type MergeProgressCallback } from './video-merge-service';
import {
...
class LongVideoChainService {
  ...
  private async mergeAndInsert(batch: LongVideoBatch): Promise<void> {
    ...
    try {
      ...
      // 合并视频
      const onProgress: MergeProgressCallback = (progress, stage) => {
        console.log(`[LongVideoChain] Merge progress: ${stage} ${progress.toFixed(1)}%`);
      };
