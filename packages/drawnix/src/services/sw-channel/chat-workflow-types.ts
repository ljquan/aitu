/**
 * Chat Workflow Types for Main Thread Client
 *
 * Types for chat workflow communication between main thread and Service Worker.
 * Mirrors the SW types but defined separately to avoid circular dependencies.
 */

import type { ChatStartParams } from './types';

// Alias for backward compatibility
export type ChatParams = Omit<ChatStartParams, 'chatId'>;
...
/**
 * Chat workflow event handlers
 */
export interface ChatWorkflowEventHandlers {
  /** Called when streaming content is received */
  onStream?: (chatId: string, content: string) => void;
  /** Called when workflow status changes */
  onStatusChange?: (chatId: string, status: ChatWorkflowStatus) => void;
  /** Called when tool calls are parsed */
  onToolCalls?: (chatId: string, toolCalls: ChatToolCall[], aiAnalysis?: string) => void;
  /** Called when a tool execution starts */
  onToolStart?: (chatId: string, toolCallId: string, toolName: string) => void;
  ...
  /** Called when workflow is recovered from storage */
  onRecovered?: (chatId: string, workflow: ChatWorkflow) => void;
}
