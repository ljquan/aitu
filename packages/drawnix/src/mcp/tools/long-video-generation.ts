...
async function executeLongVideoGeneration(
  ...
  try {
    ...
    // 通知新任务创建（状态设为 completed，因为任务已在上面创建，避免重复执行）
    options.onAddSteps?.([{
      id: firstTask.id,
      mcp: 'generate_video',
      args: { prompt: firstScript.prompt, model, size },
      description: `生成视频片段 1/${scripts.length}: ${firstScript.prompt.substring(0, 30)}...`,
      status: 'completed', // 标记为已完成，避免 AIInputBar 重复执行
    }]);

    options.onChunk?.(`\n已创建第 1 个视频生成任务，共 ${scripts.length} 个片段将串行生成。\n`);
    options.onChunk?.(`\n💡 提示：每段视频生成完成后，系统会自动提取尾帧作为下一段视频的首帧，确保画面连贯。\n`);
