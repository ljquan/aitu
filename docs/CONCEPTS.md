...
> 本文档定义了 Aitu 项目的核心概念和术语，确保团队成员在开发过程中保持一致的理解。

## 一、项目定位

**Aitu (爱图)** 是一个基于 Plait 框架构建的开源白板应用，核心定位是 **AI 图片视频创作工具 + 一体化白板**。
...
| 术语 | 英文 | 定义 | 关键文件 |
|------|------|------|----------|
| **画板 (Board)** | Board | 用户的绘图区域，包含元素集合、视口状态和主题配置 | `workspace.types.ts` |
...
| **工作区 (Workspace)** | Workspace | 管理所有文件夹和画板的顶层容器，包含 UI 状态 | `workspace.types.ts` |
...
Aitu 使用 `withXxx` 模式扩展 Plait Board 功能：
