...
7. 缓存是结果的持久化层，任务完成状态不能依赖缓存是否成功。

   - 远程 URL 缓存成功时，可以返回稳定的 `/__aitu_cache__/...` 地址，供任务队列、画布和刷新恢复使用。
   - 缓存失败时必须保留原始远程 URL，并透传 `cacheWarning`，不能把已生成任务改成失败。
   - Service Worker 可以用规范化 URL 作为缓存键和去重键，但真实请求必须保留签名参数。
   - `data:` / 原始 base64 仍应优先落到稳定虚拟路径，因为它们没有可重放的远程来源。
